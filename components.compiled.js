...
function MiniMap({
  ...
  useEffect(() => {
    ...
    Object.values(SPECIAL_POS).forEach(({
      x,
      y
    }) => {
      const px = x * tilePx;
...
function _drawGrass(ctx, w, h) {
  ...
  ctx.lineWidth = 1.5;
  [[4, 20, 4, 12], [9, 24, 9, 17], [14, 18, 14, 10], [19, 22, 19, 14], [24, 26, 24, 19], [29, 20, 29, 12], [31, 24, 31, 18], [6, 30, 6, 25], [16, 30, 16, 24], [25, 29, 25, 23]].forEach(([x1, y1, x2, y2]) => {
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x2, y2);
    ctx.stroke();
...
function _drawSchool(ctx, w, h) {
  ...
  [[5, 14], [13, 14], [21, 14], [27, 14]].forEach(([x, y]) => {
    ...
    ctx.lineWidth = 0.5;
    ctx.strokeRect(x, y, 4, 5);
  });
  ctx.fillStyle = "#5c3a1a";
  ctx.fillRect(14, 25, 6, 9);
  // 旗
  ctx.strokeStyle = "#333";
  ctx.lineWidth = 1;
  ctx.beginPath();
...
function _drawHome(ctx, w, h) {
  ...
  ctx.fillStyle = "#555";
  ctx.fillRect(21, 6, 6, 2);
  ctx.fillStyle = "#5c3a1a";
  ctx.fillRect(14, 22, 6, 12);
  // ドアノブ
  ctx.fillStyle = "#f0b060";
  ctx.beginPath();
  ctx.arc(19, 28, 1, 0, Math.PI * 2);
  ctx.fill();
  // 窓
  ctx.fillStyle = "#bfdbfe";
  ...
  ctx.strokeStyle = "#7090b0";
  ctx.lineWidth = 0.5;
  ctx.strokeRect(6, 18, 6, 6);
  ctx.strokeRect(22, 18, 6, 6);
  // 窓の十字
  ctx.beginPath();
  ctx.moveTo(9, 18);
  ctx.lineTo(9, 24);
...
const ENEMY_BATTLE_IMAGE_FILE = {
  5: "Remove_background_to_create_transparent_PNG-1776601649917.png",
  // ミニスライム
  19: "Remove_background_to_create_transparent_PNG-1776601637501.png",
  // もりのせいれい
  22: "Remove_background_to_create_transparent_PNG-1776601643472.png",
  // まよいぼたる
...
function WorldLandmarkIcon({
  ...
  useEffect(() => {
    ...
    if (kind === "castleTown") {
      ...
      ctx.fillStyle = "#b45309";
      ctx.beginPath();
      ctx.moveTo(s * 0.68, s * 0.50);
      ctx.lineTo(s * 0.84, s * 0.34);
      ctx.lineTo(s * 1.0, s * 0.50);
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = "#92400e";
    ...
    if (kind === "airDock") {
      ...
      ctx.fill();
      ctx.beginPath();
      ctx.arc(s * 0.65, s * 0.35, s * 0.06, 0, Math.PI * 2);
      ctx.fill();
      // 煙突
      ctx.fillStyle = "#6b7280";
    ...
    if (kind === "reef") {
      ...
      ctx.beginPath();
      ctx.arc(s * 0.50, s * 0.22, s * 0.12, 0, Math.PI * 2);
      ctx.fill();
      return;
    }
    ...
    if (kind === "artisan") {
      ...
      ctx.beginPath();
      ctx.arc(s * 0.39, s * 0.20, s * 0.05, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(s * 0.61, s * 0.20, s * 0.05, 0, Math.PI * 2);
      ctx.fill();
      // 扉
      ctx.fillStyle = "#1f2937";
...
function ShopOverlay({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col h-full bg-black text-white p-4 gap-3"
  }, /*#__PURE__*/React.createElement("p", {
    className: "text-center text-yellow-300 text-xs border-b border-gray-700 pb-2",
    style: {
      fontFamily: "'Courier New',monospace"
    }
  }, "\u2500\u2500 \u30B7\u30E7\u30C3\u30D7 \u2500\u2500"), /*#__PURE__*/React.createElement("p", {
    className: "text-right text-yellow-400 text-xs"
  }, player.gold, "G"), /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u305D\u3046\u3073\u3061\u3085\u3046") : /*#__PURE__*/React.createElement("span", {
      className: "flex gap-2 items-center"
    }, item.slot && /*#__PURE__*/React.createElement("span", {
      className: "text-gray-500 text-[10px]"
    }, slotLabel[item.slot]), /*#__PURE__*/React.createElement("span", {
      className: "text-gray-300"
    }, item.desc), /*#__PURE__*/React.createElement("span", {
      className: "text-yellow-300"
    }, item.cost, "G"))));
  })), msg && /*#__PURE__*/React.createElement("p", {
    className: "text-center text-green-300 text-xs animate-pulse"
  }, msg), /*#__PURE__*/React.createElement("button", {
    className: "border-2 border-white py-2 text-xs active:opacity-60",
    style: {
      fontFamily: "'Courier New',monospace"
    },
    onClick: onClose
...
function MessageBox({
  ...
  useEffect(() => {
    if (shown >= effectiveLines.length) {
      setDone(true);
      return;
    }
    if (!effectiveLines[shown]) {
      setDone(true);
      return;
    }
  ...
  const advance = () => {
    ...
    if (shown < effectiveLines.length - 1) {
      setShown(s => s + 1);
      setCharIdx(0);
    } else {
      onNext && onNext();
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    tabIndex: 0
  }, displayLines.map((l, i) => /*#__PURE__*/React.createElement("p", {
    key: i,
    className: "text-sm leading-6"
  }, l || "　")), done && showCursor && /*#__PURE__*/React.createElement("span", {
    className: "inline-block animate-bounce text-yellow-300 ml-1"
  }, "\u25BC"));
}
function HPBar({
  label,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "w-10 text-right text-gray-300"
  }, label), /*#__PURE__*/React.createElement("div", {
    className: "flex-1 bg-gray-800 border border-gray-600 h-3 rounded-sm overflow-hidden"
    ...
    className: "w-14 text-right text-white"
  }, current, "/", max));
}
...
const TITLE_IMG_BG = ["./images/title-bg.png", "./images/title-bg.jpg", "./images/title-background.png", "./images/title_sky.png", "./images/night-sky.png", "./images/bg-title.png"];
const TITLE_IMG_FG = ["./images/title-mountains.png", "./images/title-silhouette.png", "./images/title-foreground.png", "./images/title-ground.png", "./images/mountains.png", "./images/silhouette.png"];
const TITLE_IMG_LOGO = ["./images/title-logo.png", "./images/logo.png", "./images/manabi-logo.png", "./images/title.png", "./images/title-text.png"];
...
function TitleScreen({
  ...
  useEffect(() => {
    ...
    const clouds = Array.from({
      ...
      x: Math.random() * W,
      y: H * (0.08 + i * 0.04 + Math.random() * 0.06),
      w: 55 + Math.random() * 75,
    ...
    function drawDragon(x, y, f) {
      ...
      ctx.beginPath();
      ctx.ellipse(0, 0, 21, 7, -0.15, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(21, -2, 9, 5, 0.25, 0, Math.PI * 2);
      ctx.fill();
      // 翼上
      ctx.beginPath();
      ctx.moveTo(-4, -4);
      ctx.quadraticCurveTo(-14, -28, -28, -18);
      ctx.quadraticCurveTo(-18, -4, -4, -4);
      ctx.fill();
      // 翼下
      ctx.beginPath();
      ctx.moveTo(-4, 4);
      ctx.quadraticCurveTo(-14, 24 + 4 * Math.sin(f * 0.14), -24, 14);
      ctx.quadraticCurveTo(-17, 4, -4, 4);
      ctx.fill();
      // 尾
      ctx.beginPath();
      ctx.moveTo(-18, 2);
      ctx.quadraticCurveTo(-36, 7 + 3 * Math.sin(f * 0.11), -43, 0);
      ctx.strokeStyle = 'rgba(70,15,95,0.75)';
      ctx.lineWidth = 4;
      ctx.stroke();
      ctx.restore();
    }
    ...
    function loop(now = performance.now()) {
      ...
      for (let i = meteors.length - 1; i >= 0; i--) {
        ...
        ctx.fillStyle = `rgba(255,240,160,${a2})`;
        ctx.fill();
        m.x += m.vx;
        m.y += m.vy;
        m.life--;
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "relative flex flex-col items-center justify-center h-full gap-3 px-4 w-full",
    style: {
      zIndex: 10
    }
  }, /*#__PURE__*/React.createElement("p", {
  ...
  }, "\uFF5E FANTASY RPG \uFF5E"), /*#__PURE__*/React.createElement("div", {
    style: {
      position: "relative",
      textAlign: "center",
      overflow: "hidden"
    }
  }, titleLogo ? /*#__PURE__*/React.createElement("div", {
    style: {
      minHeight: 100,
      display: "flex",
      alignItems: "center",
  ...
  }, /*#__PURE__*/React.createElement("div", null, LINE1.map((ch, i) => /*#__PURE__*/React.createElement("span", {
    key: i,
    style: {
      opacity: revealChar > i ? 1 : 0,
      transition: "opacity 0.07s ease",
      display: "inline"
    }
  }, ch))), /*#__PURE__*/React.createElement("div", null, LINE2.map((ch, i) => /*#__PURE__*/React.createElement("span", {
    key: i + 4,
    style: {
      opacity: revealChar > i + 4 ? 1 : 0,
      transition: "opacity 0.07s ease",
      display: "inline"
    }
  }, ch)))), swept && !titleLogo && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute",
      top: 0,
      left: 0,
  ...
  }, "MANABI NO TABI"), /*#__PURE__*/React.createElement("div", {
    style: {
      opacity: phase >= 4 ? 1 : 0,
  ...
  }, "\u25B6 \u3064\u3065\u304D\u304B\u3089"), /*#__PURE__*/React.createElement("button", {
    onClick: handleStart,
    style: {
      fontFamily: DOT,
      fontSize: 12,
      ...
      width: '100%',
      cursor: 'pointer',
      background: blink ? 'linear-gradient(135deg,rgba(255,255,255,0.1),rgba(255,255,255,0.02))' : 'rgba(0,0,0,0.01)',
  ...
  }, "\u25B6 \u306F\u3058\u3081\u308B")), /*#__PURE__*/React.createElement("p", {
    style: {
      fontFamily: DOT,
      fontSize: 9,
      color: 'rgba(75,75,95,0.6)',
      opacity: phase >= 4 ? 1 : 0,
      transition: 'opacity 1s ease 0.5s'
    }
  }, "\xA9 2026 Titan Games \xB7 For tired adventurers")), /*#__PURE__*/React.createElement("style", null, `
...
function NameInput({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col items-center justify-center h-full bg-black text-white gap-6 px-6"
  }, /*#__PURE__*/React.createElement("div", {
    className: "border-2 border-white w-full p-6 text-center"
  }, /*#__PURE__*/React.createElement("p", {
    className: "text-sm mb-4",
    style: {
      fontFamily: "'Courier New',monospace"
    }
  }, "\u3042\u306A\u305F\u306E\u3000\u306A\u307E\u3048\u3092\u3000\u304A\u3057\u3048\u3066\u304F\u3060\u3055\u3044\u3002"), /*#__PURE__*/React.createElement("input", {
    className: "bg-black border-b-2 border-yellow-400 text-white text-center text-lg outline-none w-48 pb-1",
    style: {
      fontFamily: "'Courier New',monospace"
    },
    maxLength: 8,
    ...
    className: "border-2 border-white px-8 py-3 text-sm disabled:opacity-30",
    style: {
      fontFamily: "'Courier New',monospace"
    },
    disabled: !name.trim(),
...
function GenderScreen({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col items-center justify-center h-full bg-black text-white gap-8 px-6"
  }, /*#__PURE__*/React.createElement("div", {
    className: "border-2 border-white w-full p-6 text-center"
  }, /*#__PURE__*/React.createElement("p", {
  ...
  }, playerName), /*#__PURE__*/React.createElement("p", {
    className: "text-sm mb-8",
    style: {
      fontFamily: "'Courier New',monospace"
    }
  }, "\u3042\u306A\u305F\u306F\u3000\u7537\u306E\u5B50\uFF1F\u3000\u5973\u306E\u5B50\uFF1F"), /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-3xl"
  }, "\u2642"), /*#__PURE__*/React.createElement("span", null, "\u7537\u306E\u5B50")), /*#__PURE__*/React.createElement("button", {
    ...
    className: "text-3xl"
  }, "\u2640"), /*#__PURE__*/React.createElement("span", null, "\u5973\u306E\u5B50")))));
}
function PrologueScreen({
  ...
  const canvasRef = useRef(null);
  const lines = [`${playerName}は　目を　さました。`, "……ここは　どこだ？", "見覚えのない　広い草原。", "空の色が　少し　おかしい。", "雲が　黒ずんでいる。遠くに　暗い山が　見える。", "そのとき　声が　聞こえた。", "「……勇者よ。よく　来た。」", "「この世界は　今　ドランゴという　闇の竜に　支配されている。」", "「人々の　笑顔が　消えた。光が　消えた。」", "「どうか……この世界を　救ってほしい。」", "声は　消えた。", `${playerName}は　ゆっくり　立ち上がった。`, "まずは　誰かに　話を　聞いてみよう。"];
  useEffect(() => {
    const canvas = canvasRef.current;
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col justify-end h-full bg-black p-4 gap-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex-1 border border-gray-700 overflow-hidden"
  }, /*#__PURE__*/React.createElement("canvas", {
    ref: canvasRef,
...
function MapScreen({
  ...
  const nightOverlay = timeOfDay === "night" ? "rgba(0,15,70,0.50)" : timeOfDay === "evening" ? "rgba(100,40,0,0.28)" : timeOfDay === "morning" ? "rgba(255,160,0,0.12)" : "rgba(0,0,0,0)";
  const tileName = {
    [TILE.GRASS]: "草原",
    [TILE.FOREST]: "森",
    [TILE.WATER]: "川",
  ...
  useEffect(() => {
    const handler = e => {
      ...
      else if (e.key === "i" || e.key === "Escape" || e.key === "x") {
        e.preventDefault();
        onInfo();
      } // B = メニュー
    ...
    const pollGamepad = () => {
      ...
      if (gp && !lock) {
        ...
        if (Math.abs(axX) > 0.65 || Math.abs(axY) > 0.65) {
          lock = true;
          if (Math.abs(axX) > Math.abs(axY)) onMove(axX > 0 ? 1 : -1, 0);else onMove(0, axY > 0 ? 1 : -1);
          setTimeout(() => {
            lock = false;
          }, 110);
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col h-full bg-black text-white px-2 py-3 gap-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "border border-gray-600 p-2 text-xs grid grid-cols-2 gap-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-yellow-300"
  }, player.name, "\u3000", /*#__PURE__*/React.createElement("span", {
    className: "text-cyan-200 text-[10px]"
  }, progress.title)), /*#__PURE__*/React.createElement("span", {
    className: "text-right text-gray-400"
  }, "Lv.", player.level, "\u3000", /*#__PURE__*/React.createElement("span", {
    className: "text-blue-300"
  }, timeLabel)), /*#__PURE__*/React.createElement(HPBar, {
    label: "HP",
    current: player.hp,
    max: player.maxHp
    ...
    style: {
      gridTemplateColumns: `repeat(${VIEW_SIZE},${tileSize}px)`
    }
  }, viewRows.map(row => row.map(({
    x,
    y,
    tile
  }) => {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, /*#__PURE__*/React.createElement(TileCanvas, {
      tile: tile,
      size: tileSize
    }), /*#__PURE__*/React.createElement("span", {
      style: {
      ...
      const _vm = canUseAirship(player) && [TILE.MOUNTAIN, ..._waterTiles].includes(tile) ? "airship" : canUseShip(player) && _waterTiles.includes(tile) ? "ship" : null;
      return _vm ? /*#__PURE__*/React.createElement(VehicleSprite, {
        type: _vm,
        direction: player.direction,
        size: Math.round(tileSize * 1.3)
      }) : /*#__PURE__*/React.createElement(HeroSprite, {
        gender: player.gender,
        direction: player.direction,
        animStep: player.animStep,
        size: heroSize
      });
    })() : isSign ? /*#__PURE__*/React.createElement(SignPostIcon, {
      size: iconSize
    }) : landmarkType ? /*#__PURE__*/React.createElement(WorldLandmarkIcon, {
      kind: landmarkType,
      size: iconSize + 2
    }) : null));
  }))), nightOverlay !== "rgba(0,0,0,0)" && /*#__PURE__*/React.createElement("div", {
    className: "absolute inset-0 pointer-events-none",
    style: {
    ...
    player: player
  }), showLargeMap && /*#__PURE__*/React.createElement("div", {
    className: "absolute inset-0 z-20 flex items-center justify-center bg-black/78 pointer-events-none"
  }, /*#__PURE__*/React.createElement(MiniMap, {
    player: player,
    ...
    className: "bg-zinc-700 border border-zinc-400 text-white active:scale-95",
    onPointerDown: handleMovePointer(0, -1),
    onPointerUp: stopMoveHold,
    onPointerCancel: stopMoveHold,
    onPointerLeave: stopMoveHold
  }, "\u25B2"), /*#__PURE__*/React.createElement("div", null), /*#__PURE__*/React.createElement("button", {
    ...
    className: "bg-zinc-700 border border-zinc-400 text-white active:scale-95",
    onPointerDown: handleMovePointer(-1, 0),
    onPointerUp: stopMoveHold,
    onPointerCancel: stopMoveHold,
    onPointerLeave: stopMoveHold
  }, "\u25C0"), /*#__PURE__*/React.createElement("div", {
    ...
    className: "bg-zinc-900 border border-zinc-700 rounded-sm"
  }), /*#__PURE__*/React.createElement("button", {
    style: {
      width: dpadBtn,
      height: dpadBtn
    ...
    className: `flex flex-col justify-end items-center ${compactLayout ? "gap-1 pb-0.5" : "gap-2 pb-1"}`
  }, /*#__PURE__*/React.createElement("button", {
    className: `px-3 ${compactLayout ? "py-0.5 text-[8px]" : "py-1 text-[9px]"} bg-zinc-700 border border-zinc-400 rounded-full active:scale-95 leading-tight text-center`,
    onClick: onInfo
  }, "SELECT"), /*#__PURE__*/React.createElement("button", {
    className: `px-3 ${compactLayout ? "py-0.5 text-[8px]" : "py-1 text-[9px]"} bg-zinc-700 border border-zinc-400 rounded-full active:scale-95 leading-tight text-center`,
    onClick: () => onQuickSave && onQuickSave()
  }, "START"), /*#__PURE__*/React.createElement("button", {
    className: `px-3 ${compactLayout ? "py-0.5 text-[8px]" : "py-1 text-[9px]"} ${showLargeMap ? "bg-cyan-700 border-cyan-300 text-cyan-100" : "bg-zinc-700 border-zinc-400"} border rounded-full active:scale-95 leading-tight text-center`,
    onClick: toggleLargeMap
  }, "C")), /*#__PURE__*/React.createElement("div", {
    className: `flex gap-2 ${compactLayout ? "pb-0.5" : "pb-1"} items-end`
  }, /*#__PURE__*/React.createElement("button", {
    style: {
      width: actionBtn,
      height: actionBtn
    },
    className: "rounded-full bg-red-700 border-2 border-red-400 text-white text-sm active:scale-95",
    onClick: onInfo
...
function InteriorMapScreen({
  ...
  const map = isDungeon ? dungeonFloors && dungeonFloors[currentFloor] || dungeonFloors && dungeonFloors[1] || CAVE_FLOORS[1] : INTERIOR_MAPS[interiorType] ?? TOWN_IMAP;
  const events = isCave ? CAVE_EVENTS_BY_FLOOR[currentFloor] ?? {} : isCatCave ? CAT_CAVE_EVENTS_BY_FLOOR[currentFloor] ?? {} : INTERIOR_EVENTS[interiorType] ?? {};
  const treasures = isCave ? CAVE_TREASURES_BY_FLOOR[currentFloor] ?? {} : isCatCave ? CAT_CAVE_TREASURES_BY_FLOOR[currentFloor] ?? {} : INTERIOR_TREASURES[interiorType] ?? {};
  ...
  const tileStyle = isDungeon ? CAVE_INT_STYLE : isUnderground ? UNDERGROUND_INT_STYLE : isCastle ? CASTLE_INT_STYLE : isVillage ? VILLAGE_INT_STYLE : isTown ? TOWN_INT_STYLE : INT_STYLE;
  const title = isCave ? `くらやみの洞窟 B${currentFloor}` : isCatCave ? `猫影の洞窟 B${currentFloor}` : isUnderground ? "地下フィールド ─ ガイア団基地" : interiorType === "village" ? "はじまりの村 (内部)" : interiorType === "town" ? "いずみの街 (内部)" : interiorType === "artisanVillage" ? "職人の村 (工房街)" : interiorType === "catCave" ? "猫影の洞窟" : interiorType === "castle" ? "王城アストリア (城門〜玉座の間)" : interiorType === "manabiVillage" ? "まなびの村 (内部)" : interiorType === "nazoVillage" ? "謎の村 (内部)" : interiorType === "catVillage" ? "猫の村 (内部)" : interiorType === "southShrine" ? "南の祠" : interiorType === "shipReef" ? "潮騒の環礁" : interiorType === "skySanctum" ? "天空の祠" : "まなびの学校 (内部)";
  const [intPos, setIntPos] = useState({
    x: exitPos.x,
    y: Math.max(0, exitPos.y - 1)
  });
  ...
  const interactKeys = useMemo(() => {
    const keys = new Set();
    const add = (x, y) => {
      if (Number.isFinite(x) && Number.isFinite(y)) keys.add(`${y},${x}`);
    };
  ...
  const resolveEventForNpc = useCallback(eventKey => {
    ...
    if (interiorType === "nazoVillage" && eventKey === "2,3") {
      ...
      if (hasAll && !player.nazoSpellLearned) {
        ev = {
          messages: ["仙人マン：「……ほほう！", "全ての　謎を　解いたか！　見事じゃ！", "では　授けよう——　せんにんのかぜ！", "この風は　あらゆる　敵を　薙ぎ払う。", "MP6が　必要じゃが……それだけの　価値はある。", "そして……この　「ふるびたかぎ」も　持っていくがよい。", "ドランゴの　すみかへの　扉を　開ける　鍵じゃ。"],
          learnSpell: true,
          giveItem: "ancient_key"
        };
      } else if (player.nazoSpellLearned) {
        ev = {
          messages: hasAncientKey ? ["仙人マン：「……すでに　魔法は　授けた。", "せんにんのかぜは　使いこなせておるか？", "強い風は　使い所が　肝心じゃ。"] : ["仙人マン：「……ふるびたかぎを　まだ　持っていないのか。", "ほれ、持っていきなさい。"],
        ...
        ev = {
          messages: ["仙人マン：「……まだじゃ。", `残りの謎解き：${remaining.map(f => nameMap[f]).join("・")}`, "4人　全員の　謎を　聞いてからこい。"]
        };
      }
    }
    if (interiorType === "village" && eventKey === "1,11") {
      ...
      if (hasAllKeys && player.level >= 8 && !earned) {
        ev = {
          messages: ["語り部：「……おまえは　もう　恐れだけで　歩いていない。」", "「学び」「謎」「猫の絆」を　抱え、なお進んだ。", "いまより　おまえを『星あかりの冒険者』と呼ぼう。", "称号を　授かった！"],
          flag: "story:titleStarlight",
          mood: "hope"
        };
      } else if (earned) {
        ev = {
          messages: ["語り部：「星あかりの冒険者よ。", "短い旅でも　人の心を　照らすことはできる。」"],
          mood: "hope"
        };
      } else {
        ev = {
          messages: ["語り部：「称号は　力ではなく　歩みの証。", "3つのしるしを　抱え、さらに己を鍛えて　戻ってこい。」"]
        };
      }
    }
    if (interiorType === "catCave" && eventKey === "3,7") {
      ...
      if (hasKonnyaku) {
        ev = {
          messages: ["猫又：「にゃ。」", "猫又は　満足そうに　しっぽを振っている。"]
        };
      }
    }
    if (interiorType === "catVillage" && ev.catTalk) {
      ...
      if (eventKey === "2,2" && hasKonnyaku && !hasDragonScale) {
        ev = {
          messages: [...ev.unlockedMessages, "ネコ長：「そうじゃ……お礼に　これを。", "ドランゴの　うろこじゃ。　あやつを　倒した者が　落としていった。", "「ドラゴンのウロコ」を　てにいれた！"],
          giveItem: "dragon_scale"
        };
      } else if (eventKey === "2,2" && hasDragonScale) {
        ev = {
          messages: ["ネコ長：「ウロコは　もう　渡したにゃ。", "ドランゴを　たのんだにゃ……！"]
        };
      } else {
        ev = {
          messages: hasKonnyaku ? ev.unlockedMessages : ev.lockedMessages
        };
      }
    }
    if (interiorType === "school" && eventKey === "8,8") {
      ...
      if (hasQuest && !hasJoined) {
        ev = {
          messages: ["コウスケ（がり勉）：「……王様の使命を　受けたんだろ。", "……弱点は、おそれのない心、らしい。", "俺は　行けないけど……", "俺の分まで　頑張ってくれ。", "洞窟でピンチになったら　俺のことを思い出してくれ。", "コウスケが　こころで　ともに　戦う！"],
          flag: "story:kousuke_join",
          shop: true,
          shopItems: "T2"
        };
      }
    ...
    if (interiorType === "southShrine" && eventKey === "5,5") {
      ...
      if (!hasRecorder) {
        ev = {
          messages: ["占い婆さん：「……来たか。予言しておったぞ。", "旅人よ、この道聞きリコーダーを持っていきなさい。", "ふけば……まだ手に入れていないものの方角が、", "かすかに　聞こえてくるはずじゃ。", "「道聞きリコーダー」を　てにいれた！"],
          giveItem: "recorder"
        };
      } else {
        ev = {
          messages: ["占い婆さん：「リコーダーは　もう　持っておるじゃろう。", "ふけば……まだ集めていないものの方角が　分かるはずじゃ。", "じょうほう画面を開いて　使ってみるがよい。"]
        };
      }
    }
    if (interiorType === "shipReef" && eventKey === "2,5") {
      ...
      if (hasBeacon) {
        ev = {
          messages: ["潮騒の精：「波は　いつでも　旅人を見守っている。", "迷ったら　耳をすませ。道は　必ずある。」"]
        };
      }
    }
    if (interiorType === "skySanctum" && eventKey === "4,5") {
      ...
      if (hasBlessing) {
        ev = {
          messages: ["風祠の巫女：「風の加護は　もう　あなたと共にあります。", "あとは　信じて　進むだけです。」"]
        };
      }
    }
    return ev;
  ...
  const triggerEvent = useCallback(eventKey => {
    ...
    if (ev.heal) onHeal({
      amount: ev.heal,
      fullRecovery: !!ev.inn || ev.heal >= 999
    });
    ...
    if (tile === INT.STAIRS_DOWN && isDungeon && currentFloor < maxDungeonFloor) {
      ...
      for (let y = 0; y < nextMap.length; y++) {
        for (let x = 0; x < nextMap[y].length; x++) {
          if (nextMap[y][x] === INT.STAIRS_UP) stair = {
            x,
            y
          };
        }
    ...
    if (tile === INT.STAIRS_UP && isDungeon) {
      ...
      for (let y = 0; y < prevMap.length; y++) {
        for (let x = 0; x < prevMap[y].length; x++) {
          if (prevMap[y][x] === INT.STAIRS_DOWN) stair = {
            x,
            y
          };
        }
  ...
  const investigate = useCallback(() => {
    ...
    const forwardTile = map[fy]?.[fx];
    const passTalkTiles = new Set([INT.DESK, INT.COUNTER, INT.SHELF, INT.BOARD]);
    const forward2Key = passTalkTiles.has(forwardTile) ? `${fy + dirDelta.dy},${fx + dirDelta.dx}` : null;
    const checkKeys = [`${intPos.y},${intPos.x}`, forwardKey, ...(forward2Key ? [forward2Key] : []), ...[[-1, 0], [1, 0], [0, -1], [0, 1]].map(([dy, dx]) => `${intPos.y + dy},${intPos.x + dx}`)];
    for (const k of checkKeys) {
      ...
      if (treasures[k]) {
        const reward = onOpenTreasure ? onOpenTreasure(treasures[k]) : {
          messages: ["宝箱をあけた。"]
        };
        setIntEvent({
          messages: reward.messages || treasures[k].messages || ["宝箱をあけた。"]
        });
        return;
      }
  ...
  if (showShop) {
    return /*#__PURE__*/React.createElement("div", {
      className: "relative flex flex-col h-full bg-black text-white"
    }, /*#__PURE__*/React.createElement(ShopOverlay, {
      player: player,
      items: currentShopItems,
      onBuy: item => onBuy && onBuy(item),
  ...
  const handleIntEventDone = () => {
    ...
    const ev = intEvent;
    setIntEvent(null);
    if (ev.enemyId && onEnemyBattle) {
      onEnemyBattle(ev.enemyId);
      return;
    }
    if (ev.boss && onBoss) {
      onBoss();
  ...
  const renderCells = useViewport ? Array.from({
    length: viewH
  }, (_, vy) => Array.from({
    length: viewW
  }, (_, vx) => {
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: `relative flex flex-col h-full ${isDungeon ? "bg-gray-950" : isUnderground ? "bg-stone-950" : "bg-black"} text-white px-2 py-3 gap-2`
  }, /*#__PURE__*/React.createElement("div", {
    className: `border p-2 text-xs text-center ${isDungeon ? "border-gray-800 text-gray-400" : isUnderground ? "border-stone-700 text-amber-600" : "border-gray-600 text-yellow-300"}`
    ...
    style: {
      gridTemplateColumns: `repeat(${viewW},${TS}px)`,
      border: `1px solid ${isCave ? "#222" : isUnderground ? "#1c1008" : "#444"}`
    }
  }, renderCells.map(row => row.map(({
    x,
    y,
    tile
  }) => {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, isPlayer ? /*#__PURE__*/React.createElement(HeroSprite, {
      gender: player.gender,
      direction: intDir,
      animStep: animStep,
      size: TS - 2
      ...
      if (interiorType === "catVillage") {
        return /*#__PURE__*/React.createElement(CatNpcSprite, {
          furColor: p?.body ?? "#fca5a5",
          markColor: p?.hair ?? "#7c2d12",
          size: sz
        });
      }
      if (interiorType === "town" && npc.eventKey === "26,6") {
        return /*#__PURE__*/React.createElement(DogNpcSprite, {
          furColor: p?.body ?? "#c8834a",
          earColor: p?.hair ?? "#7c2d12",
          size: sz
        });
      }
      const role = interiorType === "castle" ? npc.eventKey === "4,10" ? "king" : npc.eventKey === "5,13" ? "princess" : npc.eventKey === "36,8" || npc.eventKey === "36,11" || npc.eventKey === "34,6" || npc.eventKey === "34,13" ? "guard" : "villager" : "villager";
      return /*#__PURE__*/React.createElement(NpcSprite, {
        bodyColor: p?.body ?? "#a3e635",
        hairColor: p?.hair ?? "#1a0a00",
        size: sz,
        role: role
      });
    })() : tile === INT.CHEST ? /*#__PURE__*/React.createElement(InteriorTileIcon, {
      kind: "chest",
      size: TS - 4,
      opened: chestOpened
    }) : tile === INT.EXIT ? /*#__PURE__*/React.createElement(InteriorTileIcon, {
      kind: "exit",
      size: TS - 4
    }) : tile === INT.FOUNTAIN ? /*#__PURE__*/React.createElement(InteriorTileIcon, {
      kind: "fountain",
      size: TS - 4
    }) : tile === INT.SHELF ? /*#__PURE__*/React.createElement(InteriorTileIcon, {
      kind: "shelf",
      size: TS - 4
    }) : tile === INT.DESK ? /*#__PURE__*/React.createElement(InteriorTileIcon, {
      kind: "desk",
      size: TS - 4
    }) : tile === INT.BOARD ? /*#__PURE__*/React.createElement(InteriorTileIcon, {
      kind: "board",
      size: TS - 4
    }) : tile === INT.COUNTER ? /*#__PURE__*/React.createElement(InteriorTileIcon, {
      kind: "counter",
      size: TS - 4
    }) : tile === INT.STAIRS_DOWN ? /*#__PURE__*/React.createElement(InteriorTileIcon, {
      kind: "stairsDown",
      size: TS - 4
  ...
  }, isDungeon ? "A/Enterで調べる（机越しOK） ／ 階段で階層移動" : "A/Enterで調べる（机越しOK） ／ 出口へ乗ると外へ"), /*#__PURE__*/React.createElement("div", {
    className: `mt-1 border-2 border-zinc-500 bg-zinc-800 rounded-md ${compactLayout ? "p-2" : "p-3"} shadow-[inset_0_2px_0_rgba(255,255,255,0.25)]`,
    style: {
      marginBottom: "max(env(safe-area-inset-bottom, 0px), 8px)"
    },
  ...
  }, "A")))), intEvent && /*#__PURE__*/React.createElement("div", {
    className: "absolute inset-x-2 bottom-3 z-20"
  }, /*#__PURE__*/React.createElement(MessageBox, {
    lines: intEvent.messages,
    onNext: handleIntEventDone,
    mood: intEvent.mood ?? "normal"
  })));
}
function EventScreen({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col justify-end h-full bg-black p-4 gap-4"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "SCENE"), /*#__PURE__*/React.createElement("p", {
    className: "text-yellow-300 text-xs"
  }, event.name))), /*#__PURE__*/React.createElement(MessageBox, {
    lines: msgs,
    onNext: onDone,
...
function BattleScreen({
  ...
  const [enemy, setEnemy] = useState(() => {
    const e = deepClone(initEnemy);
    e.curHp = initEnemy.hp;
    return e;
  });
  ...
  useEffect(() => {
    if (fearLevel > prevFearRef.current) {
      if (fearLevel === 2) addLog(`${player.name}は　きょうふに　とりつかれた！ こうげき-4！`);else if (fearLevel === 1) addLog(`${player.name}は　おそれを　感じている…… こうげき-2！`);
    }
  ...
  const doEnemyTurn = useCallback(curPHp => {
    ...
    if (mgPool.length && Math.random() < magicChance) {
      ...
      if (mg.transformFile) {
        setEnemy(e => ({
          ...e,
          formImageFile: mg.transformFile,
          formLabel: mg.transformLabel || ""
        }));
      ...
      if (mg.type === "heal") {
        setEnemy(e => ({
          ...e,
          curHp: Math.min(e.hp, e.curHp + mg.power)
        }));
  ...
  const attack = () => {
    ...
    const dmg = isCritical ? Math.max(2, Math.floor(effectiveAtk * 1.8) + rng(2, 5)) : Math.max(1, effectiveAtk - (enemy.def ?? 0) + rng(-1, 2));
  ...
  const castSpell = spell => {
    ...
    if (spell.effect === "heal") {
      ...
      const {
        damage,
        weak
      } = calcElementSpellDamage(spell, enemy);
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, playerHitFlash && /*#__PURE__*/React.createElement("div", {
    className: "absolute inset-0 z-10 pointer-events-none bg-red-500/15"
  }), /*#__PURE__*/React.createElement("div", {
    className: "border-2 border-gray-600 p-3 flex flex-col items-center gap-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: `${enemy.curHp < enemy.hp * 0.3 ? "opacity-60" : ""}`,
    style: {
    ...
    enemy: enemy,
    size: (isBoss ? 108 : 84) * 1.1,
    flash: enemyFlash
  })), /*#__PURE__*/React.createElement("p", {
    className: "text-sm font-bold"
  }, enemy.name), enemy.formLabel && /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] text-fuchsia-300"
  }, enemy.formLabel), /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] text-gray-300"
  }, "\u63A8\u5968Lv ", enemyRecommendedLevel), /*#__PURE__*/React.createElement("div", {
    className: "w-full bg-gray-800 border border-gray-600 h-3 rounded overflow-hidden"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bg-red-500 h-full transition-all duration-500",
    style: {
      width: `${enemyPct}%`
    }
  }))), isBoss && !bossIntroDone && /*#__PURE__*/React.createElement("div", {
    className: "absolute inset-0 z-20 flex items-center justify-center bg-black/85"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-center"
  }, /*#__PURE__*/React.createElement("p", {
    className: "text-red-300 text-lg tracking-widest animate-pulse"
  }, "BOSS BATTLE"), /*#__PURE__*/React.createElement("p", {
    className: "text-yellow-200 text-xs mt-2"
  }, "\u95C7\u304C\u63FA\u308C\u308B\u2026\u2026\u30C9\u30E9\u30F3\u30B4\u304C\u8972\u3044\u304B\u304B\u308B\uFF01"))), /*#__PURE__*/React.createElement("div", {
    className: "border border-gray-700 p-2 grid grid-cols-1 gap-1"
  }, /*#__PURE__*/React.createElement(HPBar, {
    label: "HP",
    current: pHp,
    max: player.maxHp
    ...
    max: player.maxMp,
    color: "blue"
  }), poisoned && /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] text-purple-300"
  }, "\u72B6\u614B\uFF1A\u3069\u304F (", poisonTurns, ")"), fearLevel === 2 && /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] text-red-400"
  }, "\u72B6\u614B\uFF1A\u304D\u3087\u3046\u3075\uFF08\u3053\u3046\u3052\u304D-4\uFF09"), fearLevel === 1 && /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] text-orange-300"
  }, "\u72B6\u614B\uFF1A\u304A\u3073\u3048\uFF08\u3053\u3046\u3052\u304D-2\uFF09"), fearDebuffTurns > 0 && /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] text-red-300"
  }, "\u3084\u307F\u306E\u3053\u3068\u3070\uFF08\u3053\u3046\u3052\u304D-3\u3001\u6B8B", fearDebuffTurns, "T\uFF09"), isBoss && bossCharging && /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] text-yellow-300"
  }, "\u30C9\u30E9\u30F3\u30B4\u304C\u3000\u95C7\u3092\u96C6\u675F\u4E2D\uFF08\u6B21\u30BF\u30FC\u30F3\u5927\u6280\uFF09"), hasCompanion && /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] text-cyan-300"
  }, "\u30B3\u30A6\u30B9\u30B1\u304C\u3000\u3068\u3082\u306B\u3000\u6226\u3063\u3066\u3044\u308B")), /*#__PURE__*/React.createElement("div", {
    ref: logRef,
    className: "border border-gray-800 p-2 bg-gray-950 h-[92px] overflow-y-auto"
  }, (() => {
    const latestMsg = arrayLast(log);
    const lastEnemyMsg = findLastCompat(log, l => l.startsWith(enemy.name));
    return log.map((l, i) => /*#__PURE__*/React.createElement("p", {
      key: `${l}-${i}`,
      className: `text-xs leading-5 ${l === latestMsg ? "text-yellow-200 font-semibold" : l === lastEnemyMsg ? "text-red-300" : "text-gray-400"}`
    }, l));
  })()), phase === "command" && (busy ? /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-center text-gray-400"
  }, "\u6575\u306E\u30BF\u30FC\u30F3\u2026") : /*#__PURE__*/React.createElement("div", {
    ...
    label: "じゅもん",
    action: () => setPhase("spell"),
    color: "border-blue-400 text-blue-400"
  }, {
    label: "どうぐ",
    action: () => setPhase("item"),
    color: player.bag && player.bag.length > 0 ? "border-green-500 text-green-400" : "border-gray-500 text-gray-500"
  }, {
    label: "にげる",
    action: flee,
    ...
    onClick: action
  }, label)))), phase === "spell" && /*#__PURE__*/React.createElement("div", {
    ...
    key: s.name,
    className: `border border-blue-600 py-2 text-xs flex justify-between px-4 ${pMp < s.mp ? "opacity-40" : ""}`,
    disabled: busy || pMp < s.mp,
    onClick: () => castSpell(s)
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-blue-300"
  }, s.secret ? "秘術 " : "", s.name), /*#__PURE__*/React.createElement("span", {
    className: "text-gray-400"
  }, "MP ", s.mp))), /*#__PURE__*/React.createElement("button", {
    className: "border border-gray-700 text-gray-500 py-2 text-xs",
    disabled: busy,
    onClick: () => setPhase("command")
  }, "\u3082\u3069\u308B")), phase === "item" && /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col gap-2"
  }, /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-center text-green-300"
  }, "\u2500\u2500 \u3069\u3046\u3050\u3092\u3000\u3048\u3089\u3079 \u2500\u2500"), (player.bag || []).filter(item => (item.heal ?? 0) > 0 || item.curePoison).length === 0 && /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-center text-gray-500"
  }, "\u3069\u3046\u3050\u304C\u3000\u306A\u3044\uFF01"), (player.bag || []).filter(item => (item.heal ?? 0) > 0 || item.curePoison).map(item => /*#__PURE__*/React.createElement("button", {
    key: item.id,
  ...
  }, item.name, " \xD7", item.count), /*#__PURE__*/React.createElement("span", {
    className: "text-gray-400"
  }, item.curePoison ? "どく回復" : `HP+${item.heal}`))), /*#__PURE__*/React.createElement("button", {
    className: "border border-gray-700 text-gray-500 py-2 text-xs",
    disabled: busy,
    onClick: () => setPhase("command")
  }, "\u3082\u3069\u308B")));
...
function InfoOverlay({
  ...
  const WARP_POINTS = [{
    name: "はじまりの村",
    pos: SPECIAL_POS.village
  }, {
    name: "いずみの街",
    pos: SPECIAL_POS.town
  }, {
    name: "職人の村",
    pos: SPECIAL_POS.artisanVillage
  }, {
    name: "まなびの学校",
    pos: SPECIAL_POS.school
  }, {
    name: "ながれぼしの家",
    pos: SPECIAL_POS.home
  }];
  const hasRoyalQuest = (player.storyFlags || []).includes("story:royalQuest");
  return /*#__PURE__*/React.createElement("div", {
    className: "absolute inset-0 bg-black/95 flex flex-col text-white z-50 min-h-0"
  }, /*#__PURE__*/React.createElement("p", {
    className: "flex-shrink-0 text-center text-yellow-300 text-sm border-b border-gray-700 pb-2 px-4 pt-4",
    style: {
      fontFamily: "'Courier New',monospace"
    }
  }, "\u2500\u2500 \u3058\u3087\u3046\u307B\u3046 \u2500\u2500"), /*#__PURE__*/React.createElement("div", {
    ...
    className: "border border-gray-700 p-3 text-xs space-y-1 shrink-0",
    style: {
      fontFamily: "'Courier New',monospace"
    }
  ...
  }, "\u79F0\u53F7\uFF1A", progress.title), /*#__PURE__*/React.createElement("p", {
    className: "text-cyan-200"
  }, "\u9032\u884C\u7387\uFF1A", progress.percent, "%\uFF08", progress.done, "/", progress.total, "\uFF09"), /*#__PURE__*/React.createElement("div", {
    className: "bg-gray-800 border border-gray-700 h-2 rounded overflow-hidden"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-[10px] text-gray-300"
  }, "\u6B21\u306E\u76EE\u7684\uFF1A", objective)), /*#__PURE__*/React.createElement("div", {
    className: "border-t border-gray-700 mt-2 pt-2 space-y-1"
  }, /*#__PURE__*/React.createElement("p", {
    className: "text-yellow-400"
  }, "\u2500\u2500 \u305D\u3046\u3073 \u2500\u2500"), /*#__PURE__*/React.createElement("p", null, "\u6B66\u5668\u3000\uFF1A", eqName(equip.weapon)), /*#__PURE__*/React.createElement("p", null, "\u9632\u5177\u3000\uFF1A", eqName(equip.armor)), /*#__PURE__*/React.createElement("p", null, "\u30A2\u30AF\u30BB\uFF1A", eqName(equip.accessory))), player.bag && player.bag.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "border-t border-gray-700 pt-2"
  }, /*#__PURE__*/React.createElement("p", null, "\u3069\u3046\u3050\uFF1A", player.bag.map(i => `${i.name}×${i.count}`).join(" / ")), player.bag.some(i => i.id === "recorder" && i.count > 0) && /*#__PURE__*/React.createElement("button", {
  ...
  }, "\u266A \u30EA\u30B3\u30FC\u30C0\u30FC\u3092\u3075\u304F")), player.nazoSpellLearned && /*#__PURE__*/React.createElement("p", {
    className: "text-blue-300 border-t border-gray-700 pt-2"
  }, "\u305B\u3093\u306B\u3093\u306E\u304B\u305C\u3000\u7FD2\u5F97\u6E08\u307F"), (player.storyFlags || []).includes("story:kousuke_join") && /*#__PURE__*/React.createElement("p", {
    className: "text-cyan-300 border-t border-gray-700 pt-2"
  }, "\u4EF2\u9593\uFF1A\u30B3\u30A6\u30B9\u30B1\u304C\u3000\u3053\u3053\u308D\u3067\u3000\u3068\u3082\u306B\u3000\u6226\u3046"), (player.bag || []).some(i => i.id === "dragon_scale" && i.count > 0) && /*#__PURE__*/React.createElement("p", {
    className: "text-yellow-300 border-t border-gray-700 pt-2"
  }, "\u4EF2\u9593\uFF1A\u732B\u306E\u3068\u3082\u3060\u3061\uFF08\u306B\u3083\uFF09"), /*#__PURE__*/React.createElement("div", {
    className: "border-t border-gray-700 mt-2 pt-2 space-y-1"
  }, /*#__PURE__*/React.createElement("p", {
    className: "text-yellow-400"
  }, "\u2500\u2500 \u3057\u3081\u3044 \u2500\u2500"), /*#__PURE__*/React.createElement("p", {
    className: `${hasRoyalQuest ? "text-green-300" : "text-gray-500"}`
  }, hasRoyalQuest ? "王の使命：受領" : "王の使命：未"))), player.level >= 2 && /*#__PURE__*/React.createElement("div", {
    className: "border border-blue-700 p-2"
  }, /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-blue-300 mb-2"
  }, "\u2500\u2500 \u65C5\u306E\u3057\u308B\u3079 \u2500\u2500"), WARP_POINTS.filter(wp => player.visited?.has(`${wp.pos.y},${wp.pos.x}`)).map(wp => /*#__PURE__*/React.createElement("button", {
    key: wp.name,
  ...
  }, wp.name, "\u3078\u3000\u79FB\u52D5"))), /*#__PURE__*/React.createElement("button", {
    className: "border-2 border-yellow-400 text-yellow-300 py-3 text-sm active:opacity-60",
    style: {
      fontFamily: "'Courier New',monospace"
    },
    onClick: onSave
  }, "\u30BB\u30FC\u30D6\u3059\u308B"), /*#__PURE__*/React.createElement("button", {
    className: "border-2 border-white py-2 text-sm active:opacity-60 shrink-0",
    style: {
      fontFamily: "'Courier New',monospace"
    },
    onClick: onClose
...
function GameOverScreen({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col items-center justify-center h-full bg-black text-white gap-8"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-center"
  }, /*#__PURE__*/React.createElement("p", {
    className: "text-2xl mb-4 tracking-widest"
  }, "GAME OVER"), /*#__PURE__*/React.createElement("h2", {
    className: "text-2xl tracking-widest mb-2",
    style: {
      fontFamily: "'Courier New',monospace"
    }
  }, "\u30B2\u30FC\u30E0\u30AA\u30FC\u30D0\u30FC"), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-gray-400"
  }, "\u3067\u3082\u3000\u305F\u3073\u3073\u3068\u306F\u3000\u307E\u305F\u3000\u305F\u3061\u3042\u304C\u308B\u3002")), /*#__PURE__*/React.createElement("button", {
    className: "border-2 border-white px-8 py-3 text-sm",
    style: {
      fontFamily: "'Courier New',monospace"
    },
    onClick: onRetry
...
function EndingScreen({
  ...
  const lines = ["ドランゴが　たおれた。", "……静かだった。", "ほんとうに　静かだった。", "暗かった空が　ゆっくりと　晴れていく。", "どこかで　子供の　笑い声が　聞こえた。", "———", "ライモンは　ひとり、　詩を　書き始めた。", "言葉が　また　動いた。　うたが　戻った。", "———", "ミズキが　窓を　開けた。", "空が　青かった。", "「……悪くない」と　小声で　言った。", "———", "ヒロシの娘が　笑った。", "ヒロシは　それを　見て　泣いた。", "———", "ネコが　にゃ、と言った。", "ただそれだけだった。　でも　十分だった。", "———", `そして　${player.name}は　草原に　立っていた。`, "……帰り方は　わからない。", "でも　今は　それでよかった。", "風が　吹いた。", hasSecretTitle ? `称号『${title}』が　夜明けの空に　輝いた。` : `称号『${title}』を　胸に　あたらしい朝へ歩き出す。`];
  useEffect(() => {
  ...
  }, /*#__PURE__*/React.createElement(DawnCanvas, {
    progress: shown / Math.max(1, lines.length - 1)
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      position: "relative",
      zIndex: 10,
  ...
  }, "ENDING"), /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: "center",
      fontFamily: "'Courier New',monospace"
    }
  }, lines.slice(0, shown + 1).map((l, i) => /*#__PURE__*/React.createElement("p", {
    key: i,
    style: {
      fontSize: "0.875rem",
      lineHeight: "1.75rem",
  ...
  }, "\u2014\u2014\u2014") : l))), shown >= lines.length - 1 && /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: "0.75rem",
